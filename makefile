...
MOUNTFLAGS := -v $(CURDIR)/config:/config -v $(CURDIR)/books:/books -v $(CURDIR)/files:/files -v $(CURDIR)/comics:/comics
...
test :
	...
	if [ "$(HOSTARCH)" = "armhf" ] || [ "$(ARCH)" != "armhf"  ]; then \
	    docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) sh -ec 'sleep 15; java -version'; \
	fi;
